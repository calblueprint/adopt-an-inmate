...
DROP FUNCTION IF EXISTS public.find_top_k_filtered_new(vector(384), integer, text, text, text[], text);
...
GRANT EXECUTE ON FUNCTION public.find_top_k_filtered_new(vector(384), integer, text, text, text, integer[]) TO anon, authenticated, service_role;
